...
\renewcommand*{\entrylocationstyle}[1]{{\fontsize{10pt}{1em}\bodyfontlight\slshape\color{text} #1}}
...
\begin{cvparagraph}
%---------------------------------------------------------
两年半工作经验，有基于C++的大型分布式NoSQL数据库项目开发经验。熟悉使用Python开发后台服务、自动化系统，进行数据分析、建模与可视化，有基于React的前端开发经验。作为主力工程师参与设计、开发风控决策规则分析系统、自动化风控解决方案生成平台、实时决策数据监控报警平台。
\end{cvparagraph}
...
%---------------------------------------------------------
  \cventry
    ...
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {参与风控数据分析(SQL, Pandas)，并构建了风控决策规则全方位分析系统(Flask, React)，包括规则解析引擎、近实时决策监控、决策规则性能指标评估、决策因子引用分析等模块，节省约80\%的分析成本}
  ...
  \cventry
    ...
    {\ } % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        ...
        \item {使用XGBoost构建决策树模型，按风控经验指标过滤决策规则生成风控方案}
...
%---------------------------------------------------------
  \cventry
    {Spider 实时决策数据监控报警平台 - 后端开发} % Job title
    {} % Organization
    {} % Location
    {\ } % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {开发报警任务执行系统， 消费风险决策引擎产生的警报(Apscheduler)，并调度执行事后聚合分析(Pandas, Spark)、决策反馈、可视化(Matplotlib)、邮件报警等任务。}
...
%---------------------------------------------------------
  \cventry
    {研发工程师{\ \cdotp\ \ }Mola大型分布式NoSQL数据库} % Job title
...
%---------------------------------------------------------
  \cventry
    {实习算法工程师{\ \cdotp\ \ }量化投资策略分析(毕业设计)} % Job title
    {荣石投资管理有限公司} % Organization
    {\ } % Location
    {2016年2月 - 2016年6月} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
...
%---------------------------------------------------------
  \cvskill
    {编程语言} % Category
    {Python, C++, Java, SQL, Shell, JavaScript} % Skills
%---------------------------------------------------------
  \cvskill
    {框架技术} % Category
    {Pandas, Flask, Celery, React, Redux, Spark} % Skills
%---------------------------------------------------------
  \cvskill
    {工具} % Category
