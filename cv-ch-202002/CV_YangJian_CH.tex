...
\position{高级软件工程师{\enskip\cdotp\enskip}上海}
...
\quote{硕士{\ \ \cdotp\ \ }上海交通大学{\quad\textbar\quad}学士{\ \ \cdotp\ \ }电子科技大学{\quad\textbar\quad}专业{\ \ \cdotp\ \ }软件工程}
...
\begin{cvparagraph}
%---------------------------------------------------------
三年工作经验。参与设计、开发支付风险控制相关系统，包括决策规则分析系统、自动化风控平台、实时交易数据监控报警平台；熟悉Python全栈开发，大数据分析、建模与可视化。参与开发基于C++的大型分布式NoSQL数据库，熟悉C++面向对象开发。
\end{cvparagraph}
...
%--------------------------------------------------------- {\ \cdotp\ \ }
  \cventry
    {决策规则分析系统} % Job title
    {PayPal - 数据决策分析与管理部} % Organization
    {2017年8月 - 至今} % Location
    {高级软件工程师} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {与全球分析师紧密合作，进行风控数据分析(SQL, Pandas)，构建决策规则分析平台(Flask, React)，包括规则解析(Java)、近实时决策监控、决策效果评估、决策因子引用分析(Neo4j)等，节省约80\%的决策分析成本}
        \item {优化风控大数据结构化存储读写性能(MySQL, ClickHouse)，支持增量/全量大数据更新}
        \item {实现异构数据源(HDFS, Teradata, MySQL)之间的数据ETL}
...
%---------------------------------------------------------
  \cventry
    {自动化风控平台} % Job title 数据分析与全栈开发
    {} % Organization
    {} % Location
    {\ } % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {构建统一数据预处理平台(Spark ETL)，进行特征统计分析与特征独热编码，整合生成训练与测试数据集}
        \item {使用XGBoost构建决策模型，生成决策规则，按风控经验指标筛选规则生成风控方案}
        \item {构建SQL风控规则翻译引擎(Druid AST)，支持用户从任意SQL规则翻译成IBM决策引擎规则}
        \item {构建自动风控方案监控与管理平台(Django, React)}
...
%---------------------------------------------------------
  \cventry
    {Mola大型分布式NoSQL数据库} % Job title
    {百度 - 基础架构部} % Organization
...
%---------------------------------------------------------
  \cventry
    {精益广告投放数据分析} % Job title
    {百度 - 生态数据产品部} % Organization
...
%---------------------------------------------------------
  \cvskill
    {框架技术} % Category
    {Pandas, Django, Flask, Spark, Celery, React} % Skills
%---------------------------------------------------------
  \cvskill
    {工具} % Category
    {Teradata, MySQL, MongoDB, ClickHouse, Git, Jenkins, Docker, Vim, Excel, Tableau} % Skills
%---------------------------------------------------------
  \cvskill
    {英文} % Category
